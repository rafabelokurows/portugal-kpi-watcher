...
hospedes_continente \
        .assign(month_year = hospedes_continente['month_year'].str.replace(' de ', ' '),
            date=hospedes_continente['month_year'].apply(lambda x: pd.to_datetime(x, format=date_format, errors='coerce')),
                valor=hospedes_continente['valor'].astype(int),
                ma_3=hospedes_continente["valor"].rolling(window=3).mean().shift(-1),
                ma_6=hospedes_continente["valor"].rolling(window=6).mean().shift(-2)) \
        .drop(columns="geocod")\
        .set_index('date') #\        .plot(figsize=(9, 5), title='teste')
```
...
def convert_to_datetime(date_str):
    ...
    # ,
    #         date=hospedes_continente['month_year'].apply(lambda x: pd.to_datetime(convert_to_datetime(), errors='coerce'))

hospedes_continente \
        .assign(month_year = hospedes_continente['month_year'].str.replace(' de ', ' '))\
        .assign(teste = hospedes_continente['month_year'].apply(lambda x: pd.to_datetime(convert_to_datetime(x))))
